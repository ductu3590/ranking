...
function categorizeTransaction(content) {
 ...
 */
export async function parseTransaction(content, amount, accountName = '') {
  console.log('\n=== PARSING TRANSACTION ===');
  ...
  if (!content || content.trim().length === 0) {
    ...
    if (accountName) {
      ...
      if (fallbackMatch.matched) {
        ...
        result.confidence = fallbackMatch.confidence - 20; // Lower confidence for fallback
  ...
  // Detect bank and direction
  result.bankDetected = detectBank(content);
  result.huongGiaoDich = detectDirection(amount);
  result.loaiGiaoDich = categorizeTransaction(content);
