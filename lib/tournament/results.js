...
function buildResolvedMatches(matches, gamesByMatchId, matchEngine, config = {}) {
  ...
  return matches.map((m) => {
    ...
    return {
      ...
      // round + match_key PHẢI đi tiếp: knockout.computeStandings dùng round để
      // tính maxRound (thiếu thì ra NaN và toàn bộ thứ hạng sai) và dùng match_key
      // để phân biệt chung kết 'F' với trận tranh hạng ba 'BRONZE'.
      round: m.round,
      match_key: m.match_key != null ? m.match_key : null,
      bracket_slot: m.bracket_slot != null ? m.bracket_slot : null,
      parent_match_id: m.parent_match_id != null ? m.parent_match_id : null,
      status: m.status,
