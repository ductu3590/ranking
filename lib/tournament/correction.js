...
function correctionImpact(match = {}, downstreamMatch = null, options = {}) {
  ...
  const targets = Array.isArray(downstreamMatch)
    ? downstreamMatch.filter(Boolean)
    : downstreamMatch ? [downstreamMatch] : [];

  if (!winnerChanged || !targets.length) {
    return { blocked: false, code: null, message: null, downstream: [] };
  }

  const downstream = targets.map((target) => ({ id: target.id, status: target.status }));
  const active = targets.find((target) => RUNNING.has(target.status));
  const finalized = targets.find((target) => target.status === 'finalized');

  if (active) {
    return {
      blocked: true,
      code: 'CORRECTION_BLOCKED_DOWNSTREAM',
      message: `Trận vòng sau (#${active.id}) đang diễn ra. Đưa trận đó về trạng thái chưa gọi rồi mới sửa được kết quả này.`,
      downstream,
    };
  }
  ...
  if (finalized) {
    return {
      blocked: true,
      code: 'CORRECTION_BLOCKED_DOWNSTREAM',
      message: `Trận vòng sau (#${finalized.id}) đã chốt kết quả. Phải huỷ chốt trận đó trước, rồi mới sửa được kết quả này.`,
      downstream,
    };
  }
