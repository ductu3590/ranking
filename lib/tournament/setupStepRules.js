...
function step1(draft, ctx) {
  ...
  if (!Number.isInteger(t.courtCount) || t.courtCount < 1 || t.courtCount > 20) {
    blockers.push(issue('COURT_COUNT_INVALID', 1, 'blocker', null, 'courtCount'));
  }
...
function step4(draft) {
  const blockers = [];
  if (draft.draw.status === 'stale') blockers.push(issue('DRAW_STALE', 4, 'blocker', null, 'draw'));
  else if (draft.draw.status !== 'draft' || !draft.draw.plan || !draft.draw.previewFingerprint) {
