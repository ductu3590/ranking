...
// Hỗ trợ cả định dạng cũ (gender-based: womens/mens/mixed1/mixed2) và mới (vòng đấu: round_1_pair_1...).
// Nguồn chân lý tổng ván con: pairSchedule.subKinds (mới) → config.subMatches (legacy) → derive từ gamesPerMatchup.
function resolveMatch(match, games, config = {}) {
  ...
  const subs = pairSubKinds?.length ? pairSubKinds
    : config.subMatches?.length ? config.subMatches
    : Array.from({ length: Number(config.gamesPerMatchup) || 4 }, (_, i) => `round_${i + 1}`);
