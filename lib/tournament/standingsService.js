...
const { knockoutPlacementLabels } = require('./qualification');
...
async function computeStageStandings(db, stage, groupId) {
  ...
  // Nhánh loại trực tiếp: nhãn thứ bậc tính theo số vòng thật (console và trang công khai dùng chung).
  const labelled = stage.schedule_format === 'knockout' ? knockoutPlacementLabels(standings, loaded.matches) : standings;
  return { schedule_format: stage.schedule_format, standings: labelled, matches: loaded.matches, tiebreak };
}
