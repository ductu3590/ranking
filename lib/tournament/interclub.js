...
function aggregateClubStandings(matches = [], entries = [], options = {}) {
  ...
  const result = [...rows.values()].map((row) => ({ ...row, entrant_id: row.club_id, diff: row.points_for - row.points_against,
    ...
    game_ratio: row.games_lost === 0 ? (row.games_won > 0 ? Infinity : 0) : row.games_won / row.games_lost,
    point_ratio: row.points_against === 0 ? (row.points_for > 0 ? Infinity : 0) : row.points_for / row.points_against,
  }));
