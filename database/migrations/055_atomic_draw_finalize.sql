-- Chốt bốc thăm nguyên tử; snapshot config đóng vai trò optimistic concurrency token.
...
BEGIN
  ...
  END IF;
  IF s.config IS DISTINCT FROM p_expected_config OR s.config->'draw'->>'status' IS DISTINCT FROM 'draft' THEN
    RAISE EXCEPTION 'DRAW_CONFLICT' USING ERRCODE = '40001';
  END IF;
  -- Không thay thế lịch hiện hữu, kể cả lịch chưa ghi điểm.
  IF EXISTS (SELECT 1 FROM public.tournament_matches WHERE group_id = p_group_id AND stage_id = p_stage_id) THEN
