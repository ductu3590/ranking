...
const { annotateDoubleElim, isDoubleElimKeySet } = require('../doubleElimKeys');
...
// Thứ hạng loại kép của setup v4 (trận mang match_key W/L/WF/LF/GF — spec Epic 1 D1 §7, ADR-007 D22).
// Không có tranh hạng ba: thua LF = hạng 3; các cặp thua lần hai ở cùng một vòng nhánh thua đồng hạng.
// Dải hạng lấy từ CẤU TRÚC (số trận mỗi vòng nhánh thua), nên hạng của cặp đã bị loại không đổi khi
// giải còn đang chạy. Cặp chưa bị loại: rank null, nhãn "Đang thi đấu".
function computeKeyedStandings(entrants, matches) {
  const annotated = annotateDoubleElim(matches);
  const lossesByRound = new Map();
  for (const item of annotated) {
    if (item.bracket === 'L') lossesByRound.set(item.bracketRound, (lossesByRound.get(item.bracketRound) || 0) + 1);
  }
  const lastL = Math.max(0, ...lossesByRound.keys());
  const placeOf = (round) => {
    let start = 3;
    for (const [other, count] of lossesByRound) if (other > round) start += count;
    return { start, end: start + lossesByRound.get(round) - 1 };
  };
  const done = (match) => (match.status === 'done' || match.status === 'finalized') && match.winner_entrant_id != null;
  const loserOf = (match) => (String(match.winner_entrant_id) === String(match.entrant_a_id) ? match.entrant_b_id : match.entrant_a_id);

  let champion = null;
  let runnerUp = null;
  const eliminatedAt = new Map();
  for (const item of annotated) {
    if (!done(item.match)) continue;
    if (item.bracket === 'GF') {
      champion = String(item.match.winner_entrant_id);
      runnerUp = String(loserOf(item.match));
    } else if (item.bracket === 'L') {
      eliminatedAt.set(String(loserOf(item.match)), item.bracketRound);
    }
  }

  const rows = entrants.map((entrant) => {
    const id = String(entrant.id);
    const base = { entrant_id: entrant.id, seed: entrant.seed || 0 };
    if (id === champion) return { ...base, rank: 1, placement_end: 1, exit_round: lastL + 3, label: 'Vô địch' };
    if (id === runnerUp) return { ...base, rank: 2, placement_end: 2, exit_round: lastL + 2, label: 'Á quân' };
    if (eliminatedAt.has(id)) {
      const round = eliminatedAt.get(id);
      const { start, end } = placeOf(round);
      return { ...base, rank: start, placement_end: end, exit_round: round, label: start === end ? `Hạng ${start}` : `Hạng ${start}–${end}` };
    }
    return { ...base, rank: null, placement_end: null, exit_round: lastL + 1, label: 'Đang thi đấu' };
  });
...
function computeStandings(stage, entrants, matches) {
  if (isDoubleElimKeySet(matches)) return computeKeyedStandings(entrants, matches);
