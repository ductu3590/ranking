...
// Cột tournaments.organizer_type chỉ nhận 'platform'|'club'; chế độ cộng đồng
// được lưu trong settings.organizer_mode ('internal'|'friendly'|'community').
function resolveOrganizerMode(row = {}) {
  if (row && row.organizer_type === 'community') return 'community';
  return (row && row.settings && row.settings.organizer_mode) || null;
}

function isPubliclyOpen(row = {}) {
  return resolveOrganizerMode(row) === 'community' && !!(row && row.settings && row.settings.open_registration === true);
}
