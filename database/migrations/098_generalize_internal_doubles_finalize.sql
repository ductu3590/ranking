-- Generalize internal doubles to every pair count >= 4, including odd counts.
-- Balanced groups: A = ceil(pair count / 2), B = floor(pair count / 2).
-- Forward-only: preserve the RPC contract, grants, and idempotency behavior.
BEGIN;
...
BEGIN
  ...
  -- Validate types separately: SQL NULL must not bypass checks, and scalar JSON
  -- must never reach jsonb_array_length/jsonb_array_elements.
  IF jsonb_typeof(v_pairs) IS DISTINCT FROM 'array'
    OR jsonb_typeof(v_selected) IS DISTINCT FROM 'array'
    OR jsonb_typeof(v_assignments) IS DISTINCT FROM 'array'
    OR jsonb_typeof(v_reserves) IS DISTINCT FROM 'array'
    OR jsonb_typeof(v_unpaired) IS DISTINCT FROM 'array' THEN
    RAISE EXCEPTION 'INTERNAL_DOUBLES_GROUP_KNOCKOUT_INVALID' USING ERRCODE = '22023';
  END IF;
  IF EXISTS (SELECT 1 FROM jsonb_array_elements(v_pairs) x
    WHERE jsonb_typeof(x->'memberIds') IS DISTINCT FROM 'array') THEN
  ...
  IF COALESCE(v_draft->'tournament'->>'organizerMode','') <> 'internal'
    ...
    OR jsonb_typeof(v_pairs) <> 'array' OR jsonb_array_length(v_pairs) < 4
    OR jsonb_typeof(v_selected) <> 'array' OR jsonb_array_length(v_selected) <> (jsonb_array_length(v_pairs) * 2)
    OR jsonb_typeof(v_assignments) <> 'array' OR jsonb_array_length(v_assignments) <> jsonb_array_length(v_pairs)
  ...
  IF v_fingerprint !~ '^[a-f0-9]{64}$' THEN RAISE EXCEPTION 'DRAW_FINGERPRINT_MISSING' USING ERRCODE = '22023'; END IF;
  IF lower(p_preview_fingerprint) <> v_fingerprint THEN RAISE EXCEPTION 'DRAW_FINGERPRINT_MISMATCH' USING ERRCODE = '40001'; END IF;

  IF EXISTS (SELECT 1 FROM jsonb_array_elements(v_selected) x WHERE jsonb_typeof(x) <> 'string' OR x #>> '{}' !~ '^[1-9][0-9]*$')
    OR (SELECT count(DISTINCT x #>> '{}') FROM jsonb_array_elements(v_selected) x) <> jsonb_array_length(v_pairs) * 2
    OR EXISTS (SELECT 1 FROM jsonb_array_elements(v_selected) x WHERE NOT EXISTS (SELECT 1 FROM public.club_members cm WHERE cm.id=(x #>> '{}')::bigint AND cm.group_id=p_group_id AND cm.is_active IS DISTINCT FROM false)) THEN
    RAISE EXCEPTION 'MEMBER_NOT_ACTIVE_IN_GROUP' USING ERRCODE = '23503';
  END IF;
  IF EXISTS (SELECT 1 FROM jsonb_array_elements(v_selected) x WHERE NOT EXISTS (SELECT 1 FROM public.athletes a JOIN public.club_members cm ON cm.id=a.legacy_club_member_id WHERE cm.id=(x #>> '{}')::bigint AND cm.group_id=p_group_id)) THEN
    RAISE EXCEPTION 'ATHLETE_IDENTITY_MISSING' USING ERRCODE = '23503';
  END IF;
  IF EXISTS (SELECT 1 FROM jsonb_array_elements(v_pairs) x WHERE jsonb_typeof(x) <> 'object' OR nullif(btrim(x->>'pairId'),'') IS NULL OR jsonb_typeof(x->'memberIds') <> 'array' OR jsonb_array_length(x->'memberIds') <> 2)
    OR (SELECT count(DISTINCT x->>'pairId') FROM jsonb_array_elements(v_pairs) x) <> jsonb_array_length(v_pairs)
    OR EXISTS (SELECT 1 FROM jsonb_array_elements(v_pairs) p, jsonb_array_elements(p->'memberIds') m WHERE jsonb_typeof(m) <> 'string' OR NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v_selected) s WHERE s #>> '{}' = m #>> '{}'))
    OR (SELECT count(DISTINCT m #>> '{}') FROM jsonb_array_elements(v_pairs) p, jsonb_array_elements(p->'memberIds') m) <> jsonb_array_length(v_pairs) * 2 THEN
    RAISE EXCEPTION 'PAIRING_INVALID' USING ERRCODE = '22023';
  END IF;
  IF EXISTS (SELECT 1 FROM jsonb_array_elements(v_assignments) a WHERE jsonb_typeof(a) <> 'object' OR a->>'entrantId' IS NULL OR a->>'groupLabel' NOT IN ('A','B') OR COALESCE(a->>'slot','') !~ '^[1-9][0-9]*$')
    OR (SELECT count(DISTINCT a->>'entrantId') FROM jsonb_array_elements(v_assignments) a) <> jsonb_array_length(v_pairs)
    ...
    OR (SELECT count(*) FROM jsonb_array_elements(v_assignments) a WHERE a->>'groupLabel'='A') <> ((jsonb_array_length(v_pairs) + 1) / 2)
    OR (SELECT count(*) FROM jsonb_array_elements(v_assignments) a WHERE a->>'groupLabel'='B') <> (jsonb_array_length(v_pairs) / 2)
    OR (SELECT count(DISTINCT a->>'slot') FROM jsonb_array_elements(v_assignments) a WHERE a->>'groupLabel'='A') <> ((jsonb_array_length(v_pairs) + 1) / 2)
    OR (SELECT count(DISTINCT a->>'slot') FROM jsonb_array_elements(v_assignments) a WHERE a->>'groupLabel'='B') <> (jsonb_array_length(v_pairs) / 2)
    OR EXISTS (SELECT 1 FROM generate_series(1,((jsonb_array_length(v_pairs) + 1) / 2)) n WHERE NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v_assignments) a WHERE a->>'groupLabel'='A' AND a->>'slot'=n::text))
    OR EXISTS (SELECT 1 FROM generate_series(1,(jsonb_array_length(v_pairs) / 2)) n WHERE NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v_assignments) a WHERE a->>'groupLabel'='B' AND a->>'slot'=n::text)) THEN
  ...
  FOR v_member IN SELECT value FROM jsonb_array_elements(v_selected) LOOP
    ...
    IF v_tournament_athlete_id IS NULL AND EXISTS (SELECT 1 FROM public.tournament_athletes WHERE group_id=p_group_id AND tournament_id=p_tournament_id AND athlete_id=v_athlete_id) THEN RAISE EXCEPTION 'TOURNAMENT_ATHLETE_CLUB_SCOPE_MISMATCH' USING ERRCODE = '23503'; END IF;
    IF v_tournament_athlete_id IS NULL THEN
      ...
      SELECT p_group_id,p_tournament_id,v_host_club_id,a.id,cm.full_name,'club_member' FROM public.club_members cm JOIN public.athletes a ON a.legacy_club_member_id=cm.id WHERE cm.id=v_member_id AND cm.group_id=p_group_id RETURNING id INTO v_tournament_athlete_id;
    END IF;
    v_member_athletes := v_member_athletes || jsonb_build_object(v_member_id::text, v_tournament_athlete_id);
    INSERT INTO public.tournament_division_roster_members(group_id,division_id,tournament_athlete_id) VALUES(p_group_id,p_division_id,v_tournament_athlete_id) ON CONFLICT DO NOTHING;
  END LOOP;
  FOR v_pair IN SELECT value FROM jsonb_array_elements(v_pairs) LOOP
    ...
    INSERT INTO public.tournament_pairs(group_id,division_id,name_snapshot,pairing_mode,status) VALUES(p_group_id,p_division_id,v_name,'manual','locked') RETURNING id INTO v_pair_id;
    FOR v_member IN SELECT value FROM jsonb_array_elements(v_pair->'memberIds') LOOP v_tournament_athlete_id := (v_member_athletes->>(v_member #>> '{}'))::bigint; INSERT INTO public.tournament_pair_members(group_id,pair_id,tournament_athlete_id) VALUES(p_group_id,v_pair_id,v_tournament_athlete_id); END LOOP;
    INSERT INTO public.tournament_entries(group_id,division_id,tournament_club_id,pair_id,name_snapshot,status) VALUES(p_group_id,p_division_id,v_host_club_id,v_pair_id,v_name,'approved') RETURNING id INTO v_entry_id;
    FOR v_member IN SELECT value FROM jsonb_array_elements(v_pair->'memberIds') LOOP
      ...
      INSERT INTO public.tournament_entry_members(group_id,entry_id,athlete_id,display_name_snapshot,roster_role) VALUES(p_group_id,v_entry_id,v_athlete_id,v_name,'player');
    END LOOP;
    v_pair_entries := v_pair_entries || jsonb_build_object(v_pair->>'pairId',v_entry_id);
  END LOOP;
  IF COALESCE(v_draft->'format'->'config'->>'thirdPlaceEnabled', 'false') NOT IN ('true', 'false') THEN RAISE EXCEPTION 'INTERNAL_DOUBLES_GROUP_KNOCKOUT_INVALID' USING ERRCODE = '22023'; END IF;
  v_third_place := COALESCE(v_draft->'format'->'config'->>'thirdPlaceEnabled', 'false') = 'true';
  INSERT INTO public.tournament_stages(group_id,tournament_id,division_id,stage_order,name,schedule_format,match_format,status,config) VALUES(p_group_id,p_tournament_id,p_division_id,1,'Vòng bảng','round_robin','simple','pending',jsonb_build_object('groupCount',2,'advancePerGroup',2,'draw',jsonb_build_object('status','locked','fingerprint',v_fingerprint))) RETURNING id INTO v_group_stage_id;
  INSERT INTO public.tournament_stages(group_id,tournament_id,division_id,stage_order,name,schedule_format,match_format,status,config) VALUES(p_group_id,p_tournament_id,p_division_id,2,'Chung kết','knockout','simple','pending',jsonb_build_object('thirdPlaceEnabled',v_third_place)) RETURNING id INTO v_knockout_stage_id;
  FOR v_assignment IN SELECT value FROM jsonb_array_elements(v_assignments) LOOP v_group_label:=v_assignment->>'groupLabel'; v_slot:=(v_assignment->>'slot')::integer; v_entry_id:=(v_pair_entries->>(v_assignment->>'entrantId'))::bigint; INSERT INTO public.tournament_stage_entrants(group_id,stage_id,division_id,entry_id,group_label,seed_in_stage) VALUES(p_group_id,v_group_stage_id,p_division_id,v_entry_id,v_group_label,v_slot); END LOOP;
  FOR v_group_label IN SELECT unnest(ARRAY['A','B']) LOOP
    FOR v_assignment IN SELECT value FROM jsonb_array_elements(v_assignments) WHERE value->>'groupLabel'=v_group_label ORDER BY (value->>'slot')::integer LOOP
      FOR v_member IN SELECT value FROM jsonb_array_elements(v_assignments) WHERE value->>'groupLabel'=v_group_label AND (value->>'slot')::integer>(v_assignment->>'slot')::integer ORDER BY (value->>'slot')::integer LOOP
        ...
        INSERT INTO public.tournament_matches(group_id,division_id,stage_id,round,bracket_slot,group_label,match_order,match_key,entry_a_id,entry_b_id,status,result_type) VALUES(p_group_id,p_division_id,v_group_stage_id,1,v_match_order,v_group_label,v_match_order,'GROUP-'||v_group_label||'-'||v_match_order,(v_pair_entries->>(v_assignment->>'entrantId'))::bigint,(v_pair_entries->>(v_member->>'entrantId'))::bigint,'pending','simple');
      END LOOP;
    END LOOP;
  END LOOP;
  INSERT INTO public.tournament_matches(group_id,division_id,stage_id,round,bracket_slot,match_order,match_key,status,result_type) VALUES(p_group_id,p_division_id,v_knockout_stage_id,1,0,0,'SF1','pending','simple') RETURNING id INTO v_sf1;
  INSERT INTO public.tournament_matches(group_id,division_id,stage_id,round,bracket_slot,match_order,match_key,status,result_type) VALUES(p_group_id,p_division_id,v_knockout_stage_id,1,1,1,'SF2','pending','simple') RETURNING id INTO v_sf2;
  INSERT INTO public.tournament_matches(group_id,division_id,stage_id,round,bracket_slot,match_order,match_key,status,result_type) VALUES(p_group_id,p_division_id,v_knockout_stage_id,2,0,2,'F','pending','simple') RETURNING id INTO v_final;
  ...
  INSERT INTO public.tournament_stage_transitions(group_id,tournament_id,division_id,source_stage_id,source_kind,source_group_label,source_rank,target_stage_id,target_match_id,target_slot) VALUES(p_group_id,p_tournament_id,p_division_id,v_group_stage_id,'group_rank','A',1,v_knockout_stage_id,v_sf1,'a'),(p_group_id,p_tournament_id,p_division_id,v_group_stage_id,'group_rank','B',2,v_knockout_stage_id,v_sf1,'b'),(p_group_id,p_tournament_id,p_division_id,v_group_stage_id,'group_rank','B',1,v_knockout_stage_id,v_sf2,'a'),(p_group_id,p_tournament_id,p_division_id,v_group_stage_id,'group_rank','A',2,v_knockout_stage_id,v_sf2,'b');
  INSERT INTO public.tournament_stage_transitions(group_id,tournament_id,division_id,source_stage_id,source_kind,source_match_id,source_outcome,target_stage_id,target_match_id,target_slot) VALUES(p_group_id,p_tournament_id,p_division_id,v_knockout_stage_id,'match_outcome',v_sf1,'winner',v_knockout_stage_id,v_final,'a'),(p_group_id,p_tournament_id,p_division_id,v_knockout_stage_id,'match_outcome',v_sf2,'winner',v_knockout_stage_id,v_final,'b');
  ...
  v_result:=jsonb_build_object('success',true,'setup_revision',d.setup_revision,'group_stage_id',v_group_stage_id,'knockout_stage_id',v_knockout_stage_id,'group_fixtures', v_match_order,'matches',jsonb_build_object('SF1',v_sf1,'SF2',v_sf2,'F',v_final,'BRONZE',v_bronze),'draw_fingerprint',v_fingerprint);
