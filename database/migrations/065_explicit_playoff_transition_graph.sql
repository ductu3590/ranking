-- Persist cross-group playoff fixtures and their entry-based transition graph.
BEGIN;

ALTER TABLE public.tournament_matches
  ADD COLUMN IF NOT EXISTS match_key text;
CREATE UNIQUE INDEX IF NOT EXISTS idx_tournament_matches_stage_match_key
  ON public.tournament_matches(group_id, stage_id, match_key)
  WHERE match_key IS NOT NULL;

CREATE TABLE IF NOT EXISTS public.tournament_stage_transitions (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  group_id bigint NOT NULL REFERENCES public.groups(id) ON DELETE CASCADE,
  tournament_id bigint NOT NULL REFERENCES public.tournaments(id) ON DELETE CASCADE,
  division_id bigint REFERENCES public.tournament_divisions(id) ON DELETE CASCADE,
  source_stage_id bigint NOT NULL REFERENCES public.tournament_stages(id) ON DELETE CASCADE,
  source_kind text NOT NULL CHECK (source_kind IN ('group_rank', 'match_outcome')),
  source_group_label text,
  source_rank integer,
  source_match_id bigint REFERENCES public.tournament_matches(id) ON DELETE CASCADE,
  source_outcome text CHECK (source_outcome IN ('winner', 'loser')),
  target_stage_id bigint NOT NULL REFERENCES public.tournament_stages(id) ON DELETE CASCADE,
  target_match_id bigint NOT NULL REFERENCES public.tournament_matches(id) ON DELETE CASCADE,
  target_slot text NOT NULL CHECK (target_slot IN ('a', 'b')),
  created_at timestamptz NOT NULL DEFAULT now(),
  CHECK (
    (source_kind = 'group_rank' AND source_group_label IS NOT NULL AND source_rank IS NOT NULL AND source_rank > 0 AND source_match_id IS NULL AND source_outcome IS NULL)
    OR (source_kind = 'match_outcome' AND source_group_label IS NULL AND source_rank IS NULL AND source_match_id IS NOT NULL AND source_outcome IS NOT NULL)
  ),
  UNIQUE (group_id, target_match_id, target_slot)
);
CREATE INDEX IF NOT EXISTS idx_tournament_stage_transitions_group_source
  ON public.tournament_stage_transitions(group_id, source_stage_id, source_group_label, source_rank)
  WHERE source_kind = 'group_rank';
CREATE INDEX IF NOT EXISTS idx_tournament_stage_transitions_match_source
  ON public.tournament_stage_transitions(group_id, source_match_id, source_outcome)
  WHERE source_kind = 'match_outcome';
ALTER TABLE public.tournament_stage_transitions DISABLE ROW LEVEL SECURITY;
...
BEGIN
  ...
  IF NOT FOUND OR source_stage.group_id<>NEW.group_id OR target_stage.group_id<>NEW.group_id OR source_stage.tournament_id<>NEW.tournament_id OR target_stage.tournament_id<>NEW.tournament_id OR target_match.group_id<>NEW.group_id OR target_match.stage_id<>NEW.target_stage_id OR target_match.division_id IS DISTINCT FROM NEW.division_id OR source_stage.division_id IS DISTINCT FROM NEW.division_id OR target_stage.division_id IS DISTINCT FROM NEW.division_id THEN
  ...
  IF NEW.source_kind='match_outcome' THEN
    ...
    IF NOT FOUND OR source_match.group_id<>NEW.group_id OR source_match.stage_id<>NEW.source_stage_id OR source_match.division_id IS DISTINCT FROM NEW.division_id THEN RAISE EXCEPTION 'STAGE_TRANSITION_SOURCE_MATCH_SCOPE_MISMATCH' USING ERRCODE='23503'; END IF;
  END IF;
  RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS trg_tournament_stage_transition_scope ON public.tournament_stage_transitions;
CREATE TRIGGER trg_tournament_stage_transition_scope BEFORE INSERT OR UPDATE ON public.tournament_stage_transitions
  FOR EACH ROW EXECUTE FUNCTION public.assert_tournament_stage_transition_scope();
...
BEGIN
  ...
  INSERT INTO public.tournament_stage_transitions(group_id,tournament_id,division_id,source_stage_id,source_kind,source_group_label,source_rank,target_stage_id,target_match_id,target_slot) VALUES
    ...
    (p_group_id,p_tournament_id,p_division_id,p_playoff_stage_id,'match_outcome',NULL,NULL,sf1,'winner',p_playoff_stage_id,final_id,'a'),
    (p_group_id,p_tournament_id,p_division_id,p_playoff_stage_id,'match_outcome',NULL,NULL,sf2,'winner',p_playoff_stage_id,final_id,'b');
...
REVOKE ALL ON TABLE public.tournament_stage_transitions FROM PUBLIC, anon, authenticated;
...
COMMENT ON TABLE public.tournament_stage_transitions IS 'Đồ thị tiến cấp tường minh từ hạng bảng hoặc kết quả trận đến từng bên nhánh.';
COMMIT;
