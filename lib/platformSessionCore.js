...
function validatePlatformSessionRecord(cookieValue, secret, now, sessionRecord, account) {
  const token = verifyPlatformSession(cookieValue, secret, now);
  if (!token || !sessionRecord || !account) return null;
  ...
  if (account.status !== 'active' || account.access_version !== token.access_version) return null;
  return token;
}

function validatePlatformSessionLookup({ cookieValue, secret, now = Date.now(), sessionResult, accountResult } = {}) {
  if (!sessionResult || sessionResult.error || !accountResult || accountResult.error) return null;
  return validatePlatformSessionRecord(cookieValue, secret, now, sessionResult.data, accountResult.data);
}
