...
| 2 | `ROSTER_EMPTY` (tổng < 2 người), `MEMBER_OUTSIDE_GROUP`, `ATHLETE_ID_MISSING` (chỉ với member), `GUEST_NAME_INVALID` (trim 2–60 ký tự), `GUEST_REF_INVALID` (normalizer đã loại trùng `clientRef`) | `INACTIVE_MEMBER_SELECTED`, `GUEST_NAME_MATCHES_MEMBER` |
