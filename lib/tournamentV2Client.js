...
export function saveDraft(draft) {
    const tournamentId = draft?.tournamentId;
    const divisionId = draft?.divisionId;
    if (!tournamentId || !divisionId) {
        const error = new Error('Cần tạo giải và nội dung trước khi lưu nháp lên máy chủ.');
        error.code = 'SETUP_DRAFT_LOCAL_ONLY';
    ...
    const organizerMode = draft?.tournament?.organizerMode;
    const body = organizerMode === 'friendly'
        ? {
            action: 'replace_invited_clubs',
            tournamentId,
            divisionId,
        ...
        : {
            ...
            athlete_ids: (draft?.participants?.selectedMemberIds || []).map(Number).filter(Number.isFinite),
            expected_setup_revision: expectedRevision,
...
export function finalize(draft) {
    return request('/setup/finalize', {
        ...
        body: {
            ...
            stagePlan: draft?.draw?.stagePlans || draft?.draw?.stage_plan || [],
            draw: draft?.draw || {},
        },
    });
}
