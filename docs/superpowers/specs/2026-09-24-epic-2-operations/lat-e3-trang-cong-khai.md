...
- **Chọn nội dung** ngay dưới header khi giải có > 1 nội dung (segmented; không chuyển trang). Trang
  `noi-dung/[division]` giữ để link cũ không vỡ, render cùng component với nội dung được chọn sẵn.
...
- Thể thức không có sơ đồ (vòng tròn thuần) → tab `Sơ đồ` ẩn.
...
| Sơ đồ | `BracketView` chỉ đọc (mobile cuộn ngang theo vòng) |
...
| `public-ui.test.js` | Đúng 4 tab + mặc định theo trạng thái; không nút quản trị; `Sơ đồ` ẩn với vòng tròn thuần; header không có ảnh poster |
