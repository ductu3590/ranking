...
function normalizeTournament(raw = {}, legacyConfig = {}) {
  const displayName = text(raw.displayName);
  const rawName = text(raw.name);
  const name = displayName || (LEGACY_PLACEHOLDER_NAMES.has(rawName) ? '' : rawName);
  return {
    name,
...
function normalizeGuests(rawGuests) {
  ...
  for (const guest of Array.isArray(rawGuests) ? rawGuests : []) {
    ...
    seen.add(clientRef);
    guests.push({ clientRef, displayName: text(guest?.displayName ?? guest?.display_name) });
  }
  return guests;
}
...
function normalizeDraft(raw) {
  ...
  const participants = {
    ...
    guests: normalizeGuests(rawParticipants.guests),
  };
  ...
  return {
    ...
    tournament: normalizeTournament(source.tournament, legacyConfig),
