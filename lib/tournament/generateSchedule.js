...
async function generateAndPersistSchedule(db, {
  ...
  const { data, error } = await db.rpc(
    finalizeDraw ? 'finalize_tournament_draw' : (entryBased ? 'replace_tournament_entry_schedule' : 'replace_tournament_schedule'),
    {
      p_group_id: groupId,
      p_stage_id: stage.id,
      p_matches: rpcMatches,
      p_idempotency_key: key,
      // CAS trong finalize RPC dùng `IS DISTINCT FROM`; null và {} là hai
      // snapshot khác nhau nên tuyệt đối không chuẩn hoá config ở đây.
      ...(finalizeDraw ? { p_expected_config: stage.config } : {}),
    },
  );
