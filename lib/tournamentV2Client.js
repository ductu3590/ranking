...
// Bốc thăm ('draw': seed mới) hoặc cập nhật xem trước ('preview': giữ seed) — server lưu luôn vào nháp.
export function drawSetup({ tournamentId, divisionId, revision, idempotencyKey, action = 'draw' }) {
    return request('/preview-schedule', {
        method: 'POST',
        body: { tournamentId: Number(tournamentId), divisionId: Number(divisionId), expectedRevision: Number(revision), idempotencyKey, action },
    });
}

export function finalizeSetup({ tournamentId, divisionId, revision, idempotencyKey, previewFingerprint }) {
    return request('/setup/finalize', {
        method: 'POST',
        body: { tournamentId: Number(tournamentId), divisionId: Number(divisionId), expectedRevision: Number(revision), idempotencyKey, previewFingerprint },
    });
}
...
// W.O. (kind 'walkover', trận đang khởi động) / bỏ cuộc (kind 'retired', đang đấu/tạm dừng) — RPC 096.
export function withdrawMatch(body) {
    return request('/withdraw', {
        method: 'POST',
        body: { ...body, idempotency_key: body?.idempotency_key || createIdempotencyKey() },
    });
}

// --- Bàn điều hành (Epic 2 E1): view model sân / hàng chờ / trận vừa chốt ---

export function getOperationsBoard(tournamentId, options = {}) {
    return request('/operations', { query: { tournamentId }, cache: 'no-store', signal: options.signal });
}
