...
export async function getValidatedPlatformSessionFromCookies() {
  try {
    ...
    const sessionResult = await supabaseAdmin.from('platform_sessions')
    ...
    const accountResult = await supabaseAdmin.from('platform_accounts')
      .select('id, role, status, access_version').eq('id', token.account_id).maybeSingle();
    const validated = validatePlatformSessionLookup({
      cookieValue: value,
      secret: secret(),
      now: Date.now(),
      sessionResult,
