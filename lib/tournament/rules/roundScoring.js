...
function describeRound(stage = {}, roundKey, totalRounds) {
  ...
  // Loại kép (setup v4): cột round là lượt thi đấu xuyên nhánh W/L; tên nhánh xem ở tab Sơ đồ.
  if (stage.schedule_format === 'double_elim') return `Lượt ${round}`;
...
// Tầng thứ năm: TRẬN, theo match_key (ADR-005 D8). Cần vì chung kết và tranh hạng ba
// cùng `round`, nên ghi đè theo vòng không tách được "chỉ chung kết chọn BO".
function readMatchOverride(stage = {}, matchKey) {
  if (!matchKey) return null;
  const table = (stage.config || {}).match_scoring;
  if (!table || typeof table !== 'object') return null;
  const raw = table[String(matchKey)];
  if (!raw || typeof raw !== 'object') return null;
  const picked = {};
  for (const field of OVERRIDABLE_FIELDS) {
    if (field in raw) picked[field] = raw[field];
...
function resolveMatchScoring(tournament, division, stage, match) {
  const roundScoring = resolveRoundScoring(tournament, division, stage, roundKeyOf(match));
  const override = readMatchOverride(stage, match?.match_key ?? match?.matchKey);
  if (!override) return roundScoring;
  const merged = { ...roundScoring, ...override };
  return {
    ...merged,
    round_source: 'match',
    engine: { ...roundScoring.engine, bestOf: Number(merged.best_of) },
  };
}
...
function computeRoundLocks(stage, matches) {
  ...
  for (const match of matches || []) {
    ...
    if (match.status === 'live' || match.started_at) entry.counts.live += 1;
