...
import { DEFAULT_GROUP_ID } from './groupConstants';
 ...
 */
async function loadMemberKeywords(groupId = DEFAULT_GROUP_ID) {
  try {
 ...
 */
async function findMemberByKeywords(content, groupId = DEFAULT_GROUP_ID) {
  const normalizedContent = normalizeForMatching(content);
  const keywords = await loadMemberKeywords(groupId);
 ...
 */
export async function parseTransaction(content, amount, accountName = '', huongGiaoDich = 'in', groupId = DEFAULT_GROUP_ID) {
  console.log('\n=== PARSING TRANSACTION ===');
  ...
  if (!content || content.trim().length === 0) {
    ...
    if (accountName) {
      const fallbackMatch = await findMemberByKeywords(accountName, groupId);
  ...
  const matchResult = await findMemberByKeywords(content, groupId);
