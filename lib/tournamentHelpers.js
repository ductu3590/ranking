...
export function calculateTeamScore(matches, teamCode) {
    console.log(`[calculateTeamScore] Called for team: ${teamCode}`);
    console.log(`[calculateTeamScore] Total matches received: ${matches.length}`);
    ...
    matches.forEach((match, index) => {
        console.log(`[calculateTeamScore] Match ${index + 1}:`, {
            id: match.id,
            round: match.round_number,
            status: match.match_status,
            winner: match.winner_team,
            statusCheck: match.match_status === 'completed',
            winnerCheck: match.winner_team === teamCode
        });

        if (match.match_status !== 'completed') {
            console.log(`  -> Skipped: match_status is "${match.match_status}", not "completed"`);
            return;
        }

        const wonMatch = match.winner_team === teamCode;
        if (!wonMatch) {
            console.log(`  -> Skipped: winner_team is "${match.winner_team}", not "${teamCode}"`);
            return;
        }

        console.log(`  -> Counting point for ${teamCode} in round ${match.round_number}`);
    ...
    console.log(`[calculateTeamScore] Final scores for ${teamCode}:`, scores);
    return scores;
}

/**
 * Check if Round 1 can be revealed based on current time
 * @param {string|any} revealTime - ISO timestamp from tournament_settings (JSONB)
 * @returns {boolean} True if current time >= reveal time
 */
export function canRevealRound1(revealTime) {
    if (!revealTime) return false;

    // Handle JSONB value from Supabase
    let timeString = revealTime;

    // If it's already a string, it might be JSON-encoded (wrapped in quotes)
    if (typeof revealTime === 'string') {
        // Remove wrapping quotes if present: "2026-01-28..." -> 2026-01-28...
        timeString = revealTime.replace(/^"(.*)"$/, '$1');
    }

    const now = new Date();
    const reveal = new Date(timeString);

    console.log('[canRevealRound1] Input:', revealTime);
    console.log('[canRevealRound1] Parsed:', timeString);
    console.log('[canRevealRound1] Current:', now.toISOString());
    console.log('[canRevealRound1] Reveal:', reveal.toISOString());
    console.log('[canRevealRound1] Should reveal:', now >= reveal);
