-- RPC chốt giải chung cho luồng tạo giải v3 (spec Lát A §10, ADR-005).
-- Route tính lại plan bằng buildSetupPlan trên server rồi truyền p_plan; RPC KHÔNG tự suy
-- cấu trúc mà kiểm bất biến rồi ghi nguyên tử. RPC v2/v3 giữ nguyên cho dữ liệu cũ.
-- Thể thức cho phép (đồng bộ với lib/tournament/setupFormats.js): group_knockout.
BEGIN;
...
DECLARE
  c_allowed_formats constant text[] := ARRAY['group_knockout'];
