-- Resolve persisted group-rank transition edges into explicit semifinal sides.
BEGIN;
...
DECLARE s public.tournament_stages%ROWTYPE; cached public.tournament_stage_advance_mutations%ROWTYPE; fingerprint text; edge public.tournament_stage_transitions%ROWTYPE; ranked jsonb; target public.tournament_matches%ROWTYPE; entry_id bigint; result jsonb; assignments integer := 0;
BEGIN
  ...
  FOR edge IN SELECT * FROM public.tournament_stage_transitions WHERE group_id=p_group_id AND source_stage_id=p_stage_id AND source_kind='group_rank' ORDER BY target_match_id,target_slot FOR UPDATE LOOP
    ...
    entry_id:=(ranked->>'entry_id')::bigint;
    IF NOT EXISTS(SELECT 1 FROM public.tournament_entries e WHERE e.id=entry_id AND e.group_id=p_group_id AND e.division_id=s.division_id AND e.status='approved') THEN RAISE EXCEPTION 'GROUP_RANK_ENTRY_SCOPE_MISMATCH' USING ERRCODE='23503'; END IF;
    SELECT * INTO target FROM public.tournament_matches WHERE id=edge.target_match_id FOR UPDATE;
    IF (edge.target_slot='a' AND target.entry_a_id IS NOT NULL AND target.entry_a_id<>entry_id) OR (edge.target_slot='b' AND target.entry_b_id IS NOT NULL AND target.entry_b_id<>entry_id) THEN RAISE EXCEPTION 'PLAYOFF_TARGET_CONFLICT' USING ERRCODE='40001'; END IF;
    UPDATE public.tournament_matches SET entry_a_id=CASE WHEN edge.target_slot='a' THEN entry_id ELSE entry_a_id END, entry_b_id=CASE WHEN edge.target_slot='b' THEN entry_id ELSE entry_b_id END WHERE id=target.id;
    INSERT INTO public.tournament_stage_entrants(group_id,stage_id,division_id,entry_id,seed_in_stage)
    VALUES(p_group_id,edge.target_stage_id,s.division_id,entry_id,CASE WHEN edge.target_slot='a' THEN 1 ELSE 2 END)
