...
function buildInternalDoublesGroupKnockoutPlan(input = {}) {
  ...
  const manualAssignments = Array.isArray(input.assignments) ? input.assignments : null;
  const groups = manualAssignments && manualAssignments.length
    ? ['A', 'B'].map((label) => ({
      label,
      entries: manualAssignments
        .filter((assignment) => String(assignment?.groupLabel ?? assignment?.group_label) === label)
        .sort((left, right) => Number(left?.slot ?? left?.seed_in_stage) - Number(right?.slot ?? right?.seed_in_stage))
        .map((assignment) => ({ entryId: String(assignment?.entrantId ?? assignment?.entry_id ?? '') })),
    }))
    : automaticGroups;
  if (manualAssignments && manualAssignments.length && (
    manualAssignments.length !== normalizedEntries.length
    || groups.some((group) => group.entries.some((entry) => !normalizedEntries.some((candidate) => candidate.entryId === entry.entryId)))
    || new Set(groups.flatMap((group) => group.entries.map((entry) => entry.entryId))).size !== normalizedEntries.length
  )) planError('MANUAL_DRAW_INVALID', 'Bản bốc thủ công phải chứa mỗi cặp đúng một lần.');
  ...
  const stages = stagePayloads.map((stage, index) => ({
    ...
    config: {
      ...(stage.config || {}),
      round_scoring: input.roundScoring && typeof input.roundScoring === 'object'
        ? (input.roundScoring[index === 0 ? 'group' : 'knockout'] || {})
        : {},
    },
