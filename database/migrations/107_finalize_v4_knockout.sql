-- Lát C: mở knockout cho finalize_internal_setup_v4 (spec Lát C §6).
-- CREATE OR REPLACE giữ nguyên signature và grants. Thay đổi so với 104:
--   1. Thêm 'knockout' vào danh sách thể thức (đồng bộ lib/tournament/setupFormats.js).
--   2. Nhánh bất biến loại trực tiếp: 1 stage knockout; n−1 trận (+1 tranh hạng ba); đúng một F;
--      mỗi ô hoặc có cặp (vòng 1 / bye) hoặc đúng một tuyến đi tới; mỗi cặp vào nhánh đúng một
--      lần; tuyến chỉ đi tới vòng sau; trận thua chỉ từ SF1/SF2 vào BRONZE; F và BRONZE không có
--      cạnh đi ra, mọi trận khác đúng một cạnh thắng đi ra.
--   3. Kiểm "trận loại trực tiếp để trống, đúng hai nguồn" chỉ áp cho group_knockout.
--   4. Stage entrants ghi vào stage mà group chỉ định (stagePlanKey, mặc định 'group-stage'):
--      nhánh loại trực tiếp không có vòng bảng nhưng standings vẫn đọc entrant của stage.
BEGIN;
