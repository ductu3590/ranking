...
const { rankStandings, ratioOf } = require('./rules/tiebreak');
...
const ATHLETE_SOURCES = new Set(['club_member', 'guest']);
...
function validateTournamentAthlete(input = {}) {
  ...
  // Nguồn VĐV được lưu tường minh, không suy ra từ athlete_id: một thành viên
  // CLB chọn từ roster nhưng chưa có bản ghi athletes (athlete_id null) vẫn là
  // club_member, không phải khách. Mặc định theo athlete_id chỉ là điểm khởi đầu.
  if (input.source != null && !ATHLETE_SOURCES.has(input.source)) fail('INVALID_ATHLETE_SOURCE', 'source must be club_member or guest');
  const source = input.source || (hasAthlete ? 'club_member' : 'guest');
  return { athlete_id: hasAthlete ? input.athlete_id : null, display_name_snapshot: displayName || null, phr_rating: input.phr_rating == null ? null : Number(input.phr_rating), phr_status: input.phr_status || 'unverified', source };
}
...
function aggregateClubStandings(matches = [], entries = [], options = {}) {
  ...
  const rows = new Map(entries.map((entry) => [String(entry.club_id), {
    club_id: entry.club_id, played: 0, won: 0, lost: 0, wins: 0,
    games_won: 0, games_lost: 0, points_for: 0, points_against: 0, diff: 0, match_points: 0,
  }]));
  ...
  for (const match of matches) {
    ...
    if (aWon) { ra.won += 1; ra.wins += 1; rb.lost += 1; ra.match_points += winPoints; }
    else { rb.won += 1; rb.wins += 1; ra.lost += 1; rb.match_points += winPoints; }
  }
  const result = [...rows.values()].map((row) => ({ ...row, entrant_id: row.club_id, diff: row.points_for - row.points_against,
    ...
    game_ratio: ratioOf(row.games_won, row.games_lost),
    point_ratio: ratioOf(row.points_for, row.points_against),
  }));
...
function buildPublicInterclubProjection({ tournament, clubs = [], divisions = [], entries = [], matches = [], games = [] } = {}) {
  ...
  return {
    ...
    // PHR công khai là tổng đã snapshot lúc duyệt entry, không phải giá trị hiện
    // tại của VĐV, và chỉ ra ngoài khi BTC bật công khai.
    entries: entries.map((row) => ({ ...pick(row, ['id', 'division_id', 'tournament_club_id', 'color_snapshot', 'seed']), name: row.name_snapshot,
      ...((tournament?.public_phr === true || tournament?.share_settings?.public_phr === true) && row.phr_total != null ? { phr_total: Number(row.phr_total) } : {}),
    })),
    ...
    games: games.map((row) => pick(row, ['id', 'match_id', 'game_no', 'kind', 'score_a', 'score_b', 'winner_entrant_id'])),
  };
}
