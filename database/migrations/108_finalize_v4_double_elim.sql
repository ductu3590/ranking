-- Epic 1 (loại kép): mở double_elimination cho finalize_internal_setup_v4 (spec Epic 1, Lát D1 §6).
-- Dựng từ 107. CREATE OR REPLACE giữ nguyên signature và grants. Thay đổi so với 107:
--   1. CHECK tournament_stages.schedule_format thêm 'double_elim' (thay CHECK cũ bằng CHECK rộng hơn
--      trong cùng transaction, mẫu 101; không đụng dữ liệu).
--   2. save_unified_setup_aggregate_draft_v1: danh sách formatKey của bản nháp thêm
--      'double_elimination' (thân hàm chép nguyên 099, chỉ đổi một dòng).
--   3. finalize: thêm 'double_elimination' vào danh sách thể thức (đồng bộ lib/tournament/setupFormats.js)
--      và 'double_elim' vào danh sách scheduleFormat của stage.
--   4. Nhánh bất biến loại kép: 1 stage double_elim không đá lại GF; 2n−2 trận (W n−1, L n−2, GF);
--      mỗi ô hoặc có cặp hoặc đúng một tuyến (không trận một bên); cặp chỉ ở nhánh thắng, mỗi cặp
--      một lần; tuyến chỉ tới lượt sau; mọi trận trừ GF đúng một cạnh thắng ra (WF/LF → GF, còn lại ở
--      lại nhánh); mọi trận nhánh thắng đúng một cạnh thua ra và chỉ vào nhánh thua; GF không có cạnh ra.
--   Phần ghi dữ liệu không đổi.
BEGIN;
