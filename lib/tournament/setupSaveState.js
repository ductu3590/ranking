...
function saveReducer(state, action) {
  switch (action.type) {
    ...
    case 'edit': {
      // Đang gõ: giữ khoảng trắng (dấu cách giữa các từ); cắt khi lưu/kiểm luật.
      const draft = normalizeDraft(typeof action.update === 'function' ? action.update(state.draft) : action.draft, { keepWhitespace: true });
    ...
    case 'saveFailure': {
      ...
      return {
        ...
        pending: null,
        // Bốc thăm dùng key riêng: không đem key đó đi thử lại một lần lưu thường.
        retryKey: untouched && action.retryable !== false ? state.pending.key : null,
      };
    }
    case 'conflictReload':
      return initialSaveState(action.payload);
