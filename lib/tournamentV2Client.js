...
export function getOperationsDashboard(tournamentId, options = {}) {
    return request('/operations/dashboard', {
        query: { tournamentId, stageId: options.stageId },
        cache: 'no-store',
    });
}

export function getCourtBoard(tournamentId, options = {}) {
    return request('/operations/court-board', {
        query: { tournamentId, stageId: options.stageId, public: options.public ? '1' : undefined },
        cache: 'no-store',
    });
}

export function updateCheckIn(body) {
    return request('/operations/check-in', { method: 'POST', body });
}

export function createScoreCorrection(body) {
    return request('/operations/corrections', {
        method: 'POST',
        body: { ...body, idempotency_key: body?.idempotency_key || createIdempotencyKey() },
    });
}

export async function listScoreCorrections(query) {
    const data = await request('/operations/corrections', { query, cache: 'no-store' });
