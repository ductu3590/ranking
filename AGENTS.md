...
**Đội hình:** `tournament-architect`, `tournament-engine-dev`, `tournament-api-dev`, `tournament-ui-dev`, `tournament-qa` (xem `.Codex/agents/`). Skill: `tournament-orchestrator`, `pickleball-formats`, `pickhub-engineering` (xem `.Codex/skills/`).
