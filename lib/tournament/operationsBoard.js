...
function buildOperationsBoard(input = {}, { now = Date.now() } = {}) {
  ...
  const queueItems = pending.map((match) => {
    ...
    return {
      ...
      projectedCourt: (courtById.get(idKey((projection.courtByMatchId || {})[match.id])) || {}).label || null,
  ...
  for (const match of [...matches].sort(byOrder(stageOrder))) {
    ...
    scheduleGroups[scheduleIndex.get(key)].matches.push({
      ...
      projectedCourt: queued ? queued.projectedCourt : null,
  ...
  return {
    ...
    stages: [...stages]
      ...
      .map((stage) => ({ id: stage.id, name: stage.name || 'Giai đoạn', format: stage.schedule_format || null, status: stage.status || null, divisionId: stage.division_id ?? null })),
