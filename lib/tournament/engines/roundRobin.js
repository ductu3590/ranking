...
function generateSchedule(stage, entrants, seed = 1) {
  ...
  const groupCount = resolveGroupCount(config);
  let order = 0;
  const matches = [];
  ...
  const assigned = entrants.filter((entry) => entry.group_label != null && entry.group_label !== '');
  if (assigned.length) {
    if (assigned.length !== entrants.length) throw new Error('INCOMPLETE_GROUP_ASSIGNMENT');
    const groups = new Map();
    for (const entry of entrants) {
      const label = String(entry.group_label);
      if (!groups.has(label)) groups.set(label, []);
      groups.get(label).push(entry);
    }
    if (groups.size !== groupCount) throw new Error('GROUP_ASSIGNMENT_COUNT_MISMATCH');
    [...groups.keys()].sort().forEach((label) => buildGroup(groups.get(label), label));
...
function computeStandings(stage, entrants, matches) {
  ...
  for (const e of entrants) {
    rows.set(e.id, {
      ...
      diff: 0, point_diff: 0, game_diff: 0, wins: 0,
      game_ratio: 0, point_ratio: 0, match_points: 0, group_label: null, seed: e.seed || 0,
    });
  }
  const h2h = new Map();
  for (const m of matches) {
    ...
    ra.played++; rb.played++;
    // Walkover vẫn là một thắng/một thua, nhưng không thể biến thành hiệu số
    // ván/điểm của bất cứ cặp nào (không có rally/game thực tế để so sánh).
    if (m.result_type !== 'walkover') {
      ra.games_won += Number(m.games_a || 0); ra.games_lost += Number(m.games_b || 0);
      rb.games_won += Number(m.games_b || 0); rb.games_lost += Number(m.games_a || 0);
      ra.points_for += Number(m.points_a || 0); ra.points_against += Number(m.points_b || 0);
      rb.points_for += Number(m.points_b || 0); rb.points_against += Number(m.points_a || 0);
    }
  ...
  for (const row of rows.values()) {
    ...
    row.point_diff = row.diff;
    row.game_diff = row.games_won - row.games_lost;
    row.wins = row.won;
    row.game_ratio = ratioOf(row.games_won, row.games_lost);
    row.point_ratio = ratioOf(row.points_for, row.points_against);
