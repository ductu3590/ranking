...
const DIVISION_PLAY_TYPES = new Set(['singles', 'doubles']);
const DIVISION_SCORING_SCOPES = new Set(['individual', 'team']);
const DIVISION_RATING_POLICIES = new Set(['open', 'cap']);
const DIVISION_PAIRING_MODES = new Set(['manual', 'random_balanced']);

function validateDivisionOptions(input = {}) {
  const value = {
    play_type: input.play_type || 'doubles',
    scoring_scope: input.scoring_scope || 'individual',
    rating_policy: input.rating_policy || 'open',
    rating_cap: input.rating_cap == null || input.rating_cap === '' ? null : Number(input.rating_cap),
    pairing_mode: input.pairing_mode || 'manual',
  ...
  if (value.rating_policy === 'cap' && (!Number.isFinite(value.rating_cap) || value.rating_cap <= 0)) fail('RATING_CAP_REQUIRED', 'rating_cap is required for cap policy');
  if (value.rating_policy === 'open') value.rating_cap = null;
  if (value.play_type === 'singles') value.pairing_mode = 'manual';
  return value;
}
...
function validateInterclubPool(entries = []) {
  const clubs = new Set();
  for (const entry of entries) {
    if (entry.club_id == null) fail('ENTRY_CLUB_REQUIRED', 'every entry needs club_id');
    if (clubs.has(String(entry.club_id))) fail('POOL_CLUB_DUPLICATE', `club ${entry.club_id} appears more than once in pool`);
    clubs.add(String(entry.club_id));
  }
  return entries.slice().sort((a, b) => (a.seed ?? Number.MAX_SAFE_INTEGER) - (b.seed ?? Number.MAX_SAFE_INTEGER));
}
