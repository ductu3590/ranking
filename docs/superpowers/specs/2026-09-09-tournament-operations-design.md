...
1. **Số sân dành cho giải** — stepper tăng/giảm số sân, ô thời lượng ước tính, ô hệ quả nói ở 5.1.
   - **Tăng**: tạo bản ghi `tournament_courts` mới, `label` tự sinh `Sân 05`…
   - **Giảm**: **không xoá bừa sân cuối danh sách.** Chỉ xoá được sân đồng thời thoả `active = false` **và** không có `tournament_match_assignments` nào trỏ tới với trận ở `pending`/`warmup`/`live`/`paused`. Không thoả → 409 `COURT_IN_USE`, kèm tên sân và số trận đang vướng, và gợi ý tắt sân thay vì xoá.
   - Sân đã từng dùng trong giải đã đấu thì **không bao giờ xoá** — tắt để giữ lịch sử.
2. **Danh sách sân** — mỗi sân một hàng: nhãn, mặt sân (`surface`), công tắc `active`. Tắt sân giữa giải thì các trận `pending` đang gán ở đó **tự đẩy về hàng đợi** (xoá `court_id` khỏi assignment, không xoá bản ghi), giờ dự kiến các trận sau dịch theo. Ghi một dòng nhật ký.
   - **Sân đang có trận `warmup`/`live`/`paused` thì không tắt được** → 409 `COURT_HAS_ACTIVE_MATCH`; BTC phải chốt hoặc chuyển trận trước.
   - **Chống ghi đè đồng thời**: cả thao tác tắt sân lẫn thao tác gán trận vào sân đều ghi có điều kiện trên `tournament_match_assignments.version` (mục 7). Riêng lệnh tắt sân dùng một câu duy nhất `UPDATE tournament_courts SET active = false WHERE id = ? AND group_id = ? AND active = true` — trả 0 dòng nghĩa là ai đó vừa tắt trước, coi như thành công (idempotent), không báo lỗi.
...
**Lý do bắt buộc ở cạnh 7 và 8.** Cạnh 6 (`live → finalized`, chốt bình thường) không cần lý do. Cạnh 7 và 8 luôn kèm `result_type` là `walkover`/`retired`, nên **bắt buộc có `reason`**; thiếu → 400 `REASON_REQUIRED`. Cạnh 2 (`warmup → pending`, huỷ gọi) cũng bắt lý do, vì nó xoá dấu vết một lần gọi sân đã phát trên mic.
...
| Route chưa commit | Xử lý | Bảng DB | Shape |
|---|---|---|---|
| `venues` | **Viết lại** | `tournament_venues` giữ nguyên | Giữ cột hiện có: `name, address, timezone, contact, map_link` |
| `courts` | **Viết lại** | `tournament_courts` giữ nguyên | Giữ `venue_id, label, surface, active`; **bỏ dùng** `availability jsonb` (không có nhu cầu) |
| `assignments` | **Viết lại** | `tournament_match_assignments` giữ nguyên | Giữ `match_id, court_id, scheduled_start, scheduled_end, version, locked`; **bỏ dùng** `time_slot_id` và `status` (trạng thái nằm ở trận, mục 6.2) |
| `schedule` | **Đổi vai** | `tournament_divisions.schedule_publication_*` | Route cũ chỉ đổi `schedule_publication_status`. Vai mới: trả **giờ dự kiến suy ra** (mục 5.4). Hai cột publication tạm để yên, dọn ở đợt gộp 4 cột trạng thái |
| `check-ins`, `qr-checkins`, `score-submissions`, `notifications`, `finance`, `time-slots` | **Xoá khỏi working tree, không commit** | bảng giữ nguyên (rỗng, vô hại) | — |
| `corrections` | **Xoá bản nháp**, viết lại ở Spec 3 | `tournament_result_corrections` giữ nguyên | Spec 3 mục 5 |

**`lib/tournament/operations.js` (56 dòng) và `app/giai-dau/v2/operations/page.js` (96 dòng): xoá ngay ở task đầu tiên của Spec 2**, không giữ làm tham chiếu. Lý do: chúng chưa từng được commit nên git không có bản ghi nào — giữ lại một file "tham chiếu" chưa commit trong working tree chỉ tạo nhầm lẫn về việc code nào đang chạy thật. Nội dung đáng giữ (bảng chuyển trạng thái) đã được chép vào mục 6.3 của spec này rồi.
