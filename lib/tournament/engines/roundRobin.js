...
function computeStandings(stage, entrants, matches) {
  ...
  for (const row of rows.values()) {
    ...
    row.wins = row.won;
    row.game_ratio = row.games_lost === 0 ? (row.games_won > 0 ? Infinity : 0) : row.games_won / row.games_lost;
    row.point_ratio = row.points_against === 0 ? (row.points_for > 0 ? Infinity : 0) : row.points_for / row.points_against;
