  ...
  return {
    ...
    async updateMembershipProfile(input) {
      ...
      if (renames) {
        ...
        const identityMap = throwOnError(mapResult);
        if (identityMap) {
          // Only full_name goes to the legacy row: club_members.aliases holds the
          // bank-transfer matching keywords and must survive a nickname edit untouched.
          throwOnError(await db.from('club_members')
            .update({ full_name: input.displayName.toUpperCase() })
