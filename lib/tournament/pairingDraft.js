...
function clone(state) {
  return { ...state, memberIds: state.memberIds.slice(), pairs: state.pairs.map((p) => ({ ...p, memberIds: p.memberIds.slice() })), unpairedMemberIds: state.unpairedMemberIds.slice() };
}
function createPairingDraft(input = {}) {
  const memberIds = [...new Set((input.memberIds || []).map(String))];
  return { memberIds, pairs: [], unpairedMemberIds: memberIds.slice(), nextPairNumber: 1 };
}
...
function pairMembers(state, pairs) {
  ...
  next.pairs = (pairs || []).map((members, index) => {
    ...
    if (ids.length !== 2 || new Set(ids).size !== 2 || ids.some((id) => used.has(id) || !next.memberIds.includes(id))) throw new Error('PAIR_MEMBER_COUNT_INVALID');
  ...
  next.unpairedMemberIds = next.memberIds.filter((id) => !used.has(id));
  return next;
}
...
function oddChoices(state) { return state.unpairedMemberIds.length % 2 ? ['add_member', 'switch_format'] : []; }
Object.assign(createPairingDraft, { pairMembers, addMember, removeMember, swapPairMembers, setLocked, regenerateUnlockedPairs, getPairs, getPair, getUnpairedMemberIds: (s) => s.unpairedMemberIds.slice(), finalizeBlockers, canSaveDraft, oddChoices });
