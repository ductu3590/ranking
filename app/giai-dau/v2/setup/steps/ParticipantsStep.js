...
export default function ParticipantsStep({ draft = {}, roster = [], onDraftChange, loading = false, error = '' }) {
    ...
    return <div className="setup-step setup-step-participants">
        ...
        <section className="participants-card" aria-labelledby="guest-participants-title">
            <div className="participants-head"><div><p className="setup-eyebrow">Mở rộng danh sách</p><h2 id="guest-participants-title">Người tham gia ngoài CLB</h2><p>Thêm khách mời để cùng thi đấu, không cần tạo thành viên CLB.</p></div><strong className="participants-count">{guests.length} khách</strong></div>
